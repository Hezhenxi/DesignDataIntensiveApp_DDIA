...
<a name="p46m-1741350285487"></a>[。配置详细和明确的监控](#40us-1708358954660)
...
<a name="jq2a-1741350285547"></a>[2.存储选择：JSON，XML,关系型数据库中表、或图模型](#zst8-1739958597323)
...
<a name="k4kb-1741350285613"></a>[图数据库：图型模型](#wjtn-1739991541917)
...
<a name="exz8-1741350285679"></a>[3.存储与检索](#sm0u-1677787447009)
...
<a name="ffwn-1741350285741"></a>[4.压缩：后台运行合并压缩过程组合段文件覆盖删除旧值](#jpws-1740164861028)
...
<a name="kw2o-1741350285804"></a>[.节省页面空间，使具有更多键，使得层次/IO更少 ps:B+树](#yhsp-1740247793034)
...
<a name="gvt2-1741350285862"></a>[将值存储在索引中](#pses-1740153582115)
...
<a name="gmfz-1741350285928"></a>[星型和雪花型:分析的模式](#ybnk-1740153582129)
...
<a name="a9wd-1741350285994"></a>[二进制编码](#vxuu-1740362391330)
...
<a name="i0f9-1741350286058"></a>[共享磁盘share-disk architecture：独立处理器和内存，数据存储在机器之间共享的磁盘阵列上，磁盘通过快速网络连接。数仓](#wgwl-1708363065800)
...
<a name="hwlq-1741350286120"></a>[解决: 用固定值替换](#rcfb-1708524371120)
...
<a name="bhbb-1741350286180"></a>[  比强一致性(同步复制/类比单机)弱，比最终一致性更强](#s6u3-1708526566643)
...
<a name="vp5y-1741350286240"></a>[记录冲突，并编写解决逻辑代码。如通过提醒用户方式](#8fcs-1708701611999)
...
<a name="gbae-1741350286299"></a>[检测并发写入](#pt1k-1708759856530)
...
<a name="xzow-1741350286364"></a>[缺点：无序，低效范围查询(需发送到所有分区)](#iktb-1709802309508)
...
<a name="5u0t-1741350286426"></a>[$hash(key)=22  22mod10..2  22mod 11...0  22 mod 9 ...4 ](#anib-1709916997775)
...
<a name="rnzr-1741350286484"></a>[具有挑战性的问题。重要的是所有参与者都同意-否则将被发送到错误的节点，而不是正确处理。分布式中的共识协议-但很难正确实现。](#1c54-1709922244119)
...
<a name="swru-1741350286546"></a>[是否需要多对象事务？ ](#t9gh-1710251706186)
...
<a name="wetj-1741350286609"></a>[为什么要防止dirty reads：](#x8ch-1710333899846)
...
<a name="ge5n-1741350286667"></a>[读已提交不提供快照隔离，保留对象的一两个版本就足够。通常支持快照隔离也是用mvcc来实现读已提交隔离级别：](#fh82-1710346453757)
...
<a name="86zz-1741350286727"></a>[原子写(x排它锁)](#lftw-1710172034957)
...
<a name="trfp-1741350286781"></a>[写入偏差与幻读（两个事务更新两个不同的对象）](#rkev-1710172453385)
...
<a name="pumy-1741350286797"></a>[写偏差的更多例子](#4jr1-1710172470885)

<a name="57fv-1741350286799"></a>[读数据为空，分别插入一条数据的会议室预定系统：select count(*) from bookings where roomid=123 and time...，insert into bookings....不能防止另一个用户插入冲突会议。解决：又需要串行化执行。](#0hyt-1710437389755)

<a name="g4s9-1741350286801"></a>[多人游戏：棋盘用锁防止丢失更新，但锁定不能妨碍将不同的棋子放在相同位置。产生写入偏差 。解决：使用唯一约束。](#pgcb-1710437636980)

<a name="nus5-1741350286803"></a>[抢注用户名：和会议系统一样。不过唯一性约束可以解决该问题。](#bwel-1710437042998)

<a name="mc89-1741350286805"></a>[防止双重开支；用户花钱和积分服务，检查支付数不超过余额，有了写入偏差，可能会发生两个支出项目同时插入，导致余额变为负值，两个事务都不会注意到另一个。](#ogt8-1710438166946)

<a name="u62u-1741350286807"></a>[导致写入偏差的幻读](#8gty-1710172478550)

<a name="qy7s-1741350286809"></a>[所有以上例子都遵循类似的模式](#tw0y-1710438423955)

<a name="m6ul-1741350286811"></a>[Select查询符合条件的行，至少两名医生值班；不存在该会议室同一时段的预定；棋盘位置未被占据；用户未被抢注；账户还有足够余额](#bngi-1710438445792)

<a name="nq4z-1741350286813"></a>[按照第一个查询的结果，应用代码决定是否继续](#l6p0-1710438590471)

<a name="dj8n-1741350286815"></a>[应用决定继续操作，执行写入(插入更新删除），并提交事务](#qt10-1710438739456)

<a name="rdhk-1741350286817"></a>[步骤3修改的行，是步骤1中返回的行之一，可以使用select for update来使事务安全避免写入偏差.](#cdmo-1710438899441)

<a name="11hu-1741350286819"></a>[但检查是否不存在满足条件行，写入添加一个匹配相同条件的行。如果步骤1中的查询没有返回任何](#caet-1710439501945)

<a name="0tbq-1741350286821"></a>[⾏，则 SELECT FOR UPDATE 锁不了任何东⻄（读已提交） 。PS:innodb可重复读读null数据会加间隙锁，不走索引则锁表。](#bsm5-1739288894816)

<a name="midu-1741350286823"></a>[       这种效应：一个事务中的写入改变另一个事务的搜索查询结果，被称为幻读。快照隔离避免了只读查询中的幻读。以上的读写事务中，幻影会导致特别棘手的写歪斜情况。](#cwgh-1710440241091)

<a name="yelh-1741350286825"></a>[物化冲突](#1cjy-1710172485201)

<a name="oc34-1741350286827"></a>[物化冲突：将幻读变为数据库中一组具体行上的锁冲突。](#fv3x-1710440426895)

<a name="jmcz-1741350286829"></a>[通过创建时间槽和房间的表，提前出入房间和时间的所有可能组合。创建预定的事务可以select for update锁定房间和时间段对应的行，在获得锁定后，可以检查重叠的预定并插入新的预定。该表不是存储预定相关的信息，它完全就是一组锁，用于防止同时修改同一房间和时间范围内的预定。](#laff-1710440634993)

<a name="bi8p-1741350286831"></a>[缺点：弄清除如何物化冲突可能很难，也很容易出错，而让并发控制机制泄露到应用数据模型是很丑陋的做法。](#dngy-1710441936216)

<a name="81os-1741350286833"></a>[物化冲突应为最后的手段。可序列化Serializeble级别是更可取的。](#i0he-1710441997817)

<a name="qbl6-1741350286835"></a>[.可序列化](#di29-1710167844061)

<a name="cp5b-1741350286837"></a>[弱隔离级别读已提交和快照隔离不能解决写入偏差幻读：](#ztac-1710587737356)

<a name="lagn-1741350286839"></a>[隔离级别难以理解，且不同数据库中实现不一致，可重复读概念不一样](#ky3h-1710588699885)

<a name="ainf-1741350286841"></a>[光检查应用代码很难判定在特定隔离界别是否安全](#tpbs-1710588738858)

<a name="5gv8-1741350286843"></a>[无检测竞争条件的好工具，往往倒霉时才会出现问题。](#jp98-1710588779920)

<a name="qfbf-1741350286845"></a>[20世纪70年代system R就已经是这样，研究人员的答案都很简单：可序列化（serializeble）级别（但非人见人爱），都使用三种技术之一。](#sqvc-1710588820273)

<a name="jlfo-1741350286847"></a>[字面意义上的串行顺序执行事务](#mqa4-1710589545518)

<a name="dzeb-1741350286849"></a>[两相锁定2PL two-phase locking,几十年来唯一的可行选择](#neot-1710589568078)

<a name="ar3b-1741350286851"></a>[乐观并发控制，可序列话的快照隔离serializable snapshot isolation](#pizj-1710589721553)

<a name="nqrv-1741350286853"></a>[真的串行执行(单线程执行事务)](#2oi3-1710172506817)

<a name="njmv-1741350286855"></a>[数据库设计人员07年才确定，单线程循环执行事务时可行的。（多线程过去30年中被认为是获得良好性能的关键所在）](#o5af-1710589822067)

<a name="dw7g-1741350286857"></a>[两个进展引发了这个反思：](#phuh-1710609564138)

<a name="ibip-1741350286859"></a>[RAM足够便宜，许多场景可以将完整的活跃数据保存在内存中。-redis](#owi7-1710609747624)

<a name="lanx-1741350286861"></a>[数据库设计人员意识到OLTP事务很短，只进行少量的读写操作。长时间的分析查询通常是只读的，可以用一致快照（快照隔离）上运行。](#vhy4-1710609817752)

<a name="vu7k-1741350286863"></a>[VoltDB/H-Store, Redis 和 Datomic中实现。避免锁的协调开销。吞吐量仅限单个cpu](#xcax-1710609934885)

<a name="1sae-1741350286865"></a>[在存储过程中封装事务(优化交互式单线程事务)](#xh4n-1710172519134)

<a name="1vht-1741350286867"></a>[数据库设计者认为，机票全流程是一个事务，可以被原子化执行。缺点：交互式会使流程性能直接下降。](#xthc-1710757083497)

<a name="scip-1741350286869"></a>[解决：单线程串行执行事务的系统不允许交互式的多语句事务。提前将整个事务作为存储过程提交数据库：](#oo8b-1710757291600)

<a name="t3uk-1741350286871"></a>[Interactive transaction :bob医生请假 1.select count  2.if count>=2 3.update false](#9fzd-1710757436562)

<a name="rcql-1741350286873"></a>[stored procedure: execute stored take doctor of call](#gegl-1710763202697)

<a name="ppvm-1741350286875"></a>[事务所需的数据都在内存中，快速执行，无需等待网络和磁盘IO](#t47s-1710763179337)

<a name="vw2z-1741350286877"></a>[存储过程的优点和缺点](#7ksv-1710172529258)

<a name="zrt2-1741350286879"></a>[名声不好：](#jfoa-1710763413471)

<a name="2sfg-1741350286881"></a>[陈旧丑陋无库生态，数据库存储过程各自的语言没跟上通用编程语言的发展](#4qhf-1710763421594)

<a name="mrjg-1741350286883"></a>[管理困难，调试困难，版本控制测试部署困难，和监控指标的系统集成困难](#96la-1710763483936)

<a name="ms0g-1741350286885"></a>[单个数据库通常由多个应用共享，写得不好的存储过程（占用大量内存和cpu）会比代码造成更大麻烦](#idii-1710763602399)

<a name="mkfe-1741350286887"></a>[ 这些问题都可以克服。现代存储过程放弃了PL/SQL,使用现有通用编程语言：VoltDB使用java或Groovy, Datomic使用Java或Clojure, Redis使用Lua.](#1w0a-1710763899035)

<a name="2djv-1741350286889"></a>[存储过程与内存存储，使得单线程执行事务可行，](#bp68-1710767504595)

<a name="fduu-1741350286891"></a>[无需等待I/O,](#ahfz-1710767568244)

<a name="uqkm-1741350286893"></a>[避免并发控制机制的开销，可以在单线程实现相当好吞吐量](#mfvt-1710767570930)

<a name="herw-1741350286895"></a>[VoltDB还用存储过程进行复制，在每个节点上执行相同的存储过程，所以要求存储过程是确定性的。比如使用当前日期，需要通过特殊的确定性API实现。](#7m1p-1710767621202)

<a name="bt8g-1741350286897"></a>[分区](#xxtp-1710172539181)

<a name="1jlj-1741350286899"></a>[顺序执行事务使并发控制简单多，但吞吐限制为单机单核。吞吐量高的应用，单线程将可能成为严重瓶颈。](#rqu5-1710767939361)

<a name="nkpw-1741350286901"></a>[扩展多核多节点，可以对数据进行分区，每个事务只需在单个分区中读写数据，事务吞度量就可以与cpu保持现行扩展。](#puih-1710768082742)

<a name="ljqg-1741350286903"></a>[对于需要访问多个分区的任务事务，需要进行协调，存储过程跨所有分区锁定执行，保证串行。](#jpiw-1710768164695)

<a name="mq3t-1741350286905"></a>[缺点：协调开销性能比单区事务慢得多。VoltDB吞吐量每秒1000个跨分区写入，比单分区低几个数量级，且通过增加机器来提高](#fgu3-1710768218238)

<a name="wx7y-1741350286907"></a>[事务是否可划分-应用的数据结构，键值数据，多个二级索的数据引需要大量跨分区协调。](#8lf4-1710768407791)

<a name="pmpw-1741350286909"></a>[串行执行小结](#zyih-1710172540516)

<a name="qm1p-1741350286911"></a>[需每个事务小而快，一个缓慢的事务拖慢所有事务。](#dibg-1710768638498)

<a name="hesc-1741350286913"></a>[可放入内存仅限活跃数据集，少量数据会移动到磁盘。但需要访问执行中的事务，系统会变慢。](#22nq-1710768681244)

<a name="fvtx-1741350286915"></a>[写入吞吐量必须低到能在单个cpu运行，否则事务能划分至但分区，且无需分区协调](#jmsk-1710768781527)

<a name="uc1b-1741350286917"></a>[跨分区事务使用程度受限。](#wlnd-1710769060180)

<a name="up53-1741350286919"></a>[两阶段锁定（2PL）](#uhxa-1710172558745)

<a name="dssl-1741350286921"></a>[30年高使用的序列化算法：两阶段锁定2PL two phase locking ,独占访问exclusive access:](#nhdj-1710769248115)

<a name="ebto-1741350286923"></a>[事务A读取对象，阻塞B写入，直至A提交；](#khdp-1710769659952)

<a name="xftd-1741350286925"></a>[事务A写入对象，阻塞B读取,直至A提交；](#elpw-1710769685611)

<a name="yeqw-1741350286927"></a>[快照隔离使得不会阻塞读，2PL提高序列化性质，解决丢失更新和写入偏差。](#6ja2-1710769959539)

<a name="uucv-1741350286929"></a>[实现两阶段锁](#lfuv-1710172605096)

<a name="ogmr-1741350286931"></a>[2PL即innoDB可序列化级别及DB2可重复读隔离级别。](#8oif-1710770052594)

<a name="1hxd-1741350286933"></a>[读写阻塞为每个对象添加锁。共享模式shared mode 与独占模式exclusive mode:](#6wad-1710770158675)

<a name="pftr-1741350286935"></a>[读取对象需以共享模式获取锁。读共享](#1dyc-1710770195788)

<a name="wslz-1741350286937"></a>[写入需独占模式取锁。写互斥。](#bshw-1710770251530)

<a name="7ztd-1741350286939"></a>[事务获得锁必须持有锁知道事务结束(提交或中止)。即一阶段获取锁，二阶段释放所有锁。](#r0b0-1711207524774)

<a name="ip7c-1741350286941"></a>[死锁deadlock, 数据库会自动检测死锁，并中止其中一个。被中止的事务充实需应用程序实现。](#iuyh-1710770211133)

<a name="mtvk-1741350286943"></a>[两阶段锁定的性能](#qi7j-1710172616303)

<a name="guca-1741350286945"></a>[比弱隔离差得多](#vz0p-1711207837446)

<a name="6rwn-1741350286947"></a>[锁开销、串行并发性降低](#ienx-1711207878742)

<a name="lq8x-1741350286949"></a>[交互式事务形成等待队列、高百分位响应慢](#z85f-1711207913718)

<a name="yher-1741350286951"></a>[死锁频繁时重试造成浪费](#kmkr-1711207799940)

<a name="8xac-1741350286953"></a>[谓词锁predicate lock](#ccq8-1710172621528)

<a name="xidi-1741350286955"></a>[谓词锁：属于符合搜索条件的对象，解决幻读phantoms](#kfpi-1711208306292)

<a name="xfhu-1741350286957"></a>[事务A获取共享谓词锁shared-mode predicate lock读取匹配对象，与排它互斥](#6dqd-1711208459688)

<a name="tdg1-1741350286959"></a>[事务A写入需等待其他事务谓词锁释放。](#p3er-1711208588906)

<a name="h9dp-1741350286961"></a>[适用于尚不存在，将可能添加的对象（幻象），如果两阶段包含谓词锁，可实现串行化。](#bdwj-1711208433268)

<a name="ngpa-1741350286963"></a>[索引范围锁next-key lock](#zv5c-1710172626404)

<a name="rmsl-1741350286965"></a>[谓词锁性能不佳：活跃事务持锁多，检查匹配非常耗时](#a3ce-1711208973949)

<a name="7oil-1741350286967"></a>[2PL采用索引范围锁-简化版谓词锁：](#ayfb-1711209059199)

<a name="izoi-1741350286969"></a>[索引位于roomid, 指示room123用于预定](#qmbu-1711212311152)

<a name="lfvf-1741350286971"></a>[或者索引位于时间上，指示12:00~13:00用于预定。](#ajbo-1711212362665)

<a name="bdyd-1741350286973"></a>[以上任何一条件，当另一个事务想写入时，需要更新索引-叶子节点，会收到共享锁。解决幻读和写入偏差](#k1ie-1711212755816)

<a name="vz9t-1741350286975"></a>[索引范围锁不如谓词锁精确，但开销低](#6lzs-1711212829915)

<a name="cryj-1741350286977"></a>[无可挂载索引，将退化为表锁，性能不利，但这是回退到一个安全位置。](#bofb-1711212877091)

<a name="9j36-1741350286979"></a>[序列化快照隔离SSI（高性能序列化算法）](#slwa-1710172573367)

<a name="e6df-1741350286981"></a>[新算法：可序列化快照隔离SSI serializeble snapshot isolation 仅与快照隔离有很小性能损失. 08年 micheal cahill博士论文。](#lnci-1711213123166)

<a name="hdrd-1741350286983"></a>[用于单节点PostgreSQL9.1后序列化和分布式数据库](#weo0-1711213307841)

<a name="38b0-1741350286985"></a>[处于实践证明阶段](#ue1s-1711213377985)

<a name="rkf2-1741350286987"></a>[悲观pessimistic与乐观optimistic的并发控制](#yq9i-1710172639651)

<a name="gtbz-1741350286989"></a>[极致悲观：串行排它锁](#54jr-1711213690005)

<a name="lgxw-1741350286991"></a>[序列化快照隔离SSI是乐观技术： ](#bjxf-1711213769533)

<a name="yltc-1741350286993"></a>[缺点：多事务争用contention时性能不佳，大部分事务中止，吞吐量极限下有重试机制将使性能变差。](#mefw-1711213819265)

<a name="bwe2-1741350286995"></a>[优点：有备用容量，竞争不高场景比悲观好，可交换的原子操作可减少争用。](#nk1s-1711213915330)

<a name="q9fa-1741350286997"></a>[SSI在快照隔离基础上添加算法检测写入的序列化冲突，并决定中止哪些事务。](#snvf-1711214019984)

<a name="nn4o-1741350286999"></a>[基于过时前提的决策](#9lgw-1710172651041)

<a name="gb2q-1741350287001"></a>[医生值班读取-修改-写入。数据库检测过时并终止：](#td4m-1711214184809)

<a name="3iyn-1741350287003"></a>[检测对旧MVCC对象版本的读取(读之前存在未提交的写入)](#t08a-1711215992016)

<a name="ryv1-1741350287005"></a>[检测影响先前读取的写入（读之后发生写入）](#aego-1711216030637)

<a name="z52l-1741350287007"></a>[检测旧MVCC读取](#5smj-1710172660675)

<a name="njca-1741350287009"></a>[事务提交时，检测是否有被忽略的写入已被提交。](#z7u2-1711216050316)

<a name="6v6w-1741350287012"></a>[SSI支持一致快照中长时间运行的读取-仅在提交时中止检测的陈旧读取。因为只读不产生幻读和写入偏差风险](#afqq-1711216243096)

<a name="rdgr-1741350287014"></a>[检测影响之前读取的写入](#mvok-1710172671381)

<a name="yznl-1741350287016"></a>[事务写入提交时，在索引中查找最近读取受影响的其他事务，通知读取的事务非最新。并中止未提交写入的其他事务。](#mo5j-1711216452745)

<a name="pe1p-1741350287018"></a>[可序列化的快照隔离的性能](#0sq6-1710172685702)

<a name="jhrt-1741350287020"></a>[粒度granularity，精准哪些事务需要中止，但簿记开销显著。](#g2eg-1711216636788)

<a name="1j4s-1741350287022"></a>[简略的跟踪速度快（粗粒度），但导致不必要的事务中止。](#ux2m-1711216812849)

<a name="8j57-1741350287024"></a>[相比2PL, 不需要锁定阻塞，读性能高](#jzqb-1711216700614)

<a name="8oqz-1741350287026"></a>[相比串行化，可扩展多台机器提高吞吐量，可跨分区读。](#qm51-1710756953994)

<a name="nwmp-1741350287028"></a>[SSI要求事务尽量短，避免长事务发生冲突中止。慢事务SSI比2PL或串行执行更不敏感](#wvsk-1711216911828)

<a name="fo6s-1741350287030"></a>[8.分布式系统的麻烦](#enui-1704985776096)

<a name="tahc-1741350287032"></a>[前几章failover故障切换, 复制延迟 与事务控制 处理各种可能发生的边缘情况，本章则是彻底的悲观总结。](#e2bp-1711387736669)

<a name="kkg5-1741350287034"></a>[故障与部分失效](#4yaw-1711386647072)

<a name="ep1x-1741350287036"></a>[部分失效partial failure  不确定性的nondeterministic: IDC配电单元故障](#9d0g-1711387883443)

<a name="zmiy-1741350287038"></a>[云计算与超级计算机](#w7ln-1711386736921)

<a name="82c2-1741350287040"></a>[不可靠的网络(Timeout)](#ynhs-1711386665148)

<a name="zdtu-1741350287042"></a>[请求丢失： 拔网线](#fav6-1711388032544)

<a name="7lrp-1741350287044"></a>[请求排队：](#znsf-1711388048086)

<a name="b2dk-1741350287046"></a>[请求失效：](#gmdu-1711388084834)

<a name="ef8w-1741350287048"></a>[.....](#dssu-1711388054944)

<a name="wtp8-1741350287050"></a>[真实世界的网络故障](#y933-1711387087536)

<a name="buja-1741350287052"></a>[检测故障](#aywv-1711387099045)

<a name="tldj-1741350287054"></a>[超时与无穷的延迟](#z4hi-1711387103669)

<a name="s0br-1741350287056"></a>[级联失效cascading failure](#qs2e-1711388153448)

<a name="hhyd-1741350287058"></a>[网络拥塞和排队](#gttn-1711387323883)

<a name="y6qz-1741350287060"></a>[TCP执行流量控制flow control](#v9f2-1711388199023)

<a name="z8rt-1741350287062"></a>[视频、语音会议使用UDP，折中丢包与流畅](#up7k-1711389082773)

<a name="lqio-1741350287064"></a>[同步网络vs异步网络](#t56p-1711387113980)

<a name="gi3b-1741350287066"></a>[有限延迟bounded delay：同步网络，不收排队影响](#fmrl-1711389283437)

<a name="ziji-1741350287068"></a>[我们不能简单地使用网络延迟可预测吗](#bwvp-1711387335617)

<a name="aecb-1741350287070"></a>[不可靠的时钟](#l3p1-1711386676597)

<a name="t2tn-1741350287072"></a>[机器的时钟：硬件设备，石英振荡器，](#faqp-1711469591901)

<a name="gncl-1741350287074"></a>[不是完全准确的。](#xtjc-1711469629964)

<a name="ywez-1741350287076"></a>[同步时钟：网络时间协议NTP](#knwp-1711469656643)

<a name="n1ag-1741350287078"></a>[服务器从精确时间源获取时间-GPS接收器](#pfpq-1711469689783)

<a name="cdn1-1741350287080"></a>[单调钟与时钟](#qoxv-1711387218692)

<a name="fjp7-1741350287082"></a>[时钟](#wu2k-1711387376627)

<a name="gjbn-1741350287084"></a>[Linux: clock_gettime(CLOCK_REALTIME)](#obwb-1711469781129)

<a name="5ivi-1741350287086"></a>[java:system.currentTimeMilis](#uxdp-1711469821742)

<a name="hl7b-1741350287088"></a>[通常与NTP同步，但举例NTP服务器太远，则可能被强制重置(及时跳回）](#gwyx-1711469840905)

<a name="nofw-1741350287090"></a>[单调钟](#hl1a-1711387385739)

<a name="4byq-1741350287092"></a>[保证总是前进](#uzaz-1711470052827)

<a name="vpnx-1741350287094"></a>[Linux:CLOCK_MONOTONIC](#tbmn-1711469988760)

<a name="xp8k-1741350287096"></a>[java:System.nanoTime](#fb70-1711470014393)

<a name="3wjs-1741350287098"></a>[调整频率：NTP检测到计算机石英钟更快或更慢，则调整单调钟频率（偏移skewing时钟）.](#sn42-1711470028035)

<a name="zln6-1741350287100"></a>[不假定不同节点时钟存在同步：分布式系统用单调钟效果好，对测量的不准确不敏感](#ct3v-1711470667953)

<a name="plu5-1741350287102"></a>[时钟同步与准确性](#a7gy-1711387238026)

<a name="mrje-1741350287104"></a>[漂移drifts: 计算机石英钟不准确原因,取决机器温度。](#pbyu-1711470750697)

<a name="bf9a-1741350287106"></a>[实现精确度：GPS接收机，精确时间协议PTP](#kgdy-1711470917769)

<a name="w3p8-1741350287108"></a>[同步错误的原因：NTP守护进程配置错误，防火墙阻止NTP通信。](#kgez-1711470944635)

<a name="sfz8-1741350287110"></a>[依赖同步时钟](#vo0i-1711387245745)

<a name="0m2s-1741350287112"></a>[使用需要同步时钟的软件需监测所有机器间的时钟偏移](#zm2f-1711471000801)

<a name="tzyk-1741350287114"></a>[有序事件的时间戳](#fnxq-1711387475761)

<a name="1abo-1741350287116"></a>[写入排序问题：B客户端写入比A晚但具有更早的时间戳。 ](#n1pp-1711471283186)

<a name="nfi7-1741350287118"></a>[ 最后写入为准LWW](#50kp-1711471399605)

<a name="cukq-1741350287120"></a>[ 版本向量](#axgx-1711471482711)

<a name="tn4y-1741350287122"></a>[时钟读数存在置信区间](#ofpq-1711387431462)

<a name="dg4z-1741350287124"></a>[ 置信区间的不确定性：NTP服务器的不去定性，加上服务器网络往返时间](#aihs-1711471513794)

<a name="uh3m-1741350287126"></a>[当clock_gettime()返回值不会告诉你预期错误，不知道置信区间是5毫秒还是5年](#aave-1711471693187)

<a name="2qmb-1741350287128"></a>[例外：Spanner Google TureTime API明确报告本地时钟置信区间（最早，最晚）。](#znqm-1711471741654)

<a name="e6dk-1741350287130"></a>[全局快照的同步时钟](#bfz6-1711387449885)

<a name="ewqo-1741350287132"></a>[分布式的事务快照隔离，分布式事务ID时序成为站不住脚瓶颈](#dpco-1711471815493)

<a name="aeqv-1741350287134"></a>[Spanner通过置信区间来比较A,B发生的时序。](#2ihe-1711471939317)

<a name="vtyw-1741350287136"></a>[分布式事务语义使用时钟同步是一个活跃的研究领域，没有在谷歌之外的主流数据库中实现。](#9qfk-1711471991096)

<a name="5iju-1741350287138"></a>[暂停进程](#3mnj-1711387250438)

<a name="ohgl-1741350287140"></a>[如何确保分布式主节点仍未领导者](#ui0e-1711472636558)

<a name="tkq6-1741350287142"></a>[方式一：从其他节点获得租约lease, 类似一个带超时的锁，周期性过期前续约。-和redison续约类似](#yh9a-1711472775035)

<a name="ad1t-1741350287144"></a>[过期时，由另一个节点接管。](#kqwv-1711472837428)

<a name="ndxa-1741350287146"></a>[缺点：依赖同步时钟](#gi5a-1711473874947)

<a name="ohuo-1741350287148"></a>[意外的时间停顿：](#93uw-1711473992955)

<a name="dwnn-1741350287150"></a>[stop-the-world  HotSpot GC](#7kp7-1711474017909)

<a name="vyem-1741350287152"></a>[suspend虚拟机](#6ejp-1711474059672)

<a name="mntd-1741350287154"></a>[随意暂停恢复：关闭笔记本盖子](#92se-1711474080346)

<a name="adgs-1741350287156"></a>[窃取时间steal time:虚拟机切换或cpu上下文切换，在其他机器中花费的时间.](#tfxw-1711474128862)

<a name="hte9-1741350287158"></a>[同步时的阻塞io：java惰性加载类文件；磁盘为网络文件系统或网络块设备-亚马逊EBS](#7jwg-1711474280466)

<a name="baqi-1741350287160"></a>[磁盘交换内存抖动thrashing: 允许交换到磁盘（分页），内存访问可能导致页面错误page fault .话费大量时间将页面交换到内存，实际完成的工作很少thrashing。 服务器机器禁用页面调度，干掉进程释放内存。](#kvsa-1711474441136)

<a name="pbnr-1741350287162"></a>[SIGSTOP暂停unix进程:Ctrl-Z立即阻止进程的CPU周期](#ocvk-1711474648160)

<a name="8ann-1741350287164"></a>[这些都可以随时抢占preempt正在运行的线程。](#t1w7-1711474768157)

<a name="4qxq-1741350287166"></a>[响应时间保证](#cirf-1711473855327)

<a name="ri60-1741350287168"></a>[硬实时hard real-time系统: 不满足特定截止时间deadline可能会导致整个系统故障。](#r1ji-1711474887529)

<a name="b0oc-1741350287170"></a>[嵌入式系统精心设计和测试：安全气囊不会因系统GC暂停弹出。](#pgjw-1711475001376)

<a name="rjz3-1741350287172"></a>[大量的额外工作，昂贵。](#urp5-1711475089330)

<a name="w0qi-1741350287174"></a>[限制垃圾收集的影响](#i1wt-1711387521748)

<a name="6mcb-1741350287176"></a>[通知GC暂停，停止接收新请求-对延迟敏感的金融系统](#oxal-1711475134772)

<a name="jr1p-1741350287178"></a>[变种：GC只收集新生代，积累老年代，一次重启一个节点，类似滚动升级。](#ho3v-1711475217820)

<a name="bh5e-1741350287180"></a>[知识、真相与谎言](#ujyf-1711386681865)

<a name="y9zf-1741350287182"></a>[真理由多数所定义](#hdwi-1711387268312)

<a name="f4x3-1741350287184"></a>[stop the world 一分钟，被其他节点宣告死亡，重新选举。](#rxcy-1711561420884)

<a name="3xnv-1741350287186"></a>[领导者与锁定](#3zea-1711387542106)

<a name="yhky-1741350287188"></a>[数据库分区领导者只能有一个节点：避免脑裂](#b6am-1711561462773)

<a name="iof3-1741350287190"></a>[特定资源的锁和对象只允许一个事务/客户端持有](#3ynw-1711561525662)

<a name="rlb6-1741350287192"></a>[用户唯一标识](#hqa3-1711561556083)

<a name="eac2-1741350287194"></a>[节点认为它是天选者the choosen one](#rv2w-1711561875596)

<a name="oz6b-1741350287196"></a>[HBase例子：分布式锁的实现不正确，客户端1持有锁stop the world导致lease超时,客户端2获得锁进行处理，客户端1仍认为它具有有效的租约，即使它已经过期，从而破坏了存储的文件。](#gu25-1711561617808)

<a name="ndps-1741350287198"></a>[防护令牌](#mhlf-1711387550649)

<a name="w5wk-1741350287200"></a>[解决天选者问题：防护fencing](#gw5w-1711561950975)

<a name="0el9-1741350287202"></a>[fencing token :只允许增加屏蔽令牌的顺序进行写操作。](#vi5w-1711562021406)

<a name="iual-1741350287204"></a>[服务器端检查token,获取锁时token增加33+1，存储服务器记录更高令牌34的写入，拒绝令牌33的请求。-一般是redisson续期，这个更像乐观锁。](#2yxl-1711562140276)

<a name="j5yk-1741350287206"></a>[ZooKeeper将事务标识zxid和节点版本conersion用作屏蔽令牌。保证了单调递增](#e6n2-1711562077138)

<a name="bsix-1741350287208"></a>[如文件存储服务器可将令牌卸载文件命中](#uujk-1711564755459)

<a name="vcp9-1741350287210"></a>[拜占庭故障](#bsay-1711387279845)

<a name="uide-1741350287212"></a>[拜占庭故障Byzantine fault: 在不信任的环境中达成共识的问题称为拜占庭将军问题。](#id0b-1711628556885)

<a name="rkbm-1741350287214"></a>[拜占庭将军问题：n位将军需要同意，一些叛徒通过发送虚假或不真实信息来混淆欺骗他人。](#lxw6-1711628612728)

<a name="uubl-1741350287216"></a>[拜占庭容错：系统在部分节点发生故障、不遵守协议，恶意攻击，扰乱网络时仍能继续正确工作：](#lpn6-1711628716132)

<a name="5bee-1741350287218"></a>[航空航天环境计算机被辐射破坏，不可预知方式响应其他节点](#py6z-1711638469892)

<a name="yvrq-1741350287220"></a>[多个参与组织系统中，参与者试图欺骗他人。比特币等区块链对等网络让互不信任的各方同意交易不依赖中央当局。](#csli-1711638605446)

<a name="5rbx-1741350287222"></a>[弱谎言形式](#f38o-1711387557457)

<a name="1oyh-1741350287224"></a>[添加防止撒谎弱形式机制；](#eepx-1711638744038)

<a name="mxfx-1741350287226"></a>[校验和防止数据包被破坏](#yseh-1711638821915)

<a name="z3f6-1741350287228"></a>[公开访问的系统需仔细清理来自用户的任何输入](#2a8h-1711638847924)

<a name="pp4y-1741350287230"></a>[NTP多数对时间范围达成一致，配置错误的服务器将被排除](#zcdt-1711638888096)

<a name="qqfw-1741350287232"></a>[系统模型与实现](#x4lu-1711387293192)

<a name="p0er-1741350287234"></a>[定时假设三种系统模型：](#iyps-1711639179162)

<a name="kdia-1741350287236"></a>[同步模型synchronous model: 网络延迟，暂停和时钟漂移不会超过某个固定上限；](#pgo0-1711640511485)

<a name="5pg9-1741350287238"></a>[部分同步模型partial synchronous：大多数情况像一个同步系统一样运行，有时出现超时，进程暂停和时钟漂移界限](#h1x4-1711640383000)

<a name="x4w4-1741350287240"></a>[异步模型：一个算法不允许对实际做任何假设，事实上它甚至没有时钟。](#vkkn-1711640459345)

<a name="cm6z-1741350287242"></a>[节点系统模型：-节点失效问题](#lg5f-1711640536166)

<a name="adit-1741350287244"></a>[崩溃停止crash-stop:只能由崩溃失效，且永远不会回来。](#ahco-1711640498506)

<a name="8jx0-1741350287246"></a>[崩溃恢复crash-recovery: 具有稳定存储（非易失性磁盘），崩溃中保留，内存中状态会丢失-innodb三阶段提交](#35nl-1711640632666)

<a name="7rvi-1741350287248"></a>[拜占庭（任意）故障：节点可以做任何事情，包括戏弄和欺骗其他节点](#rpmc-1711640822813)

<a name="iwq8-1741350287250"></a>[具有崩溃恢复的部分同步模型是最有用的模型。分布式算法如何应用这种模型](#6gea-1711640879281)

<a name="ibet-1741350287252"></a>[算法的正确性](#hwoz-1711387576180)

<a name="oj8b-1741350287254"></a>[描述属性：定义算法的正确性](#i9kv-1711640972087)

<a name="kcgo-1741350287256"></a>[锁生成屏蔽令牌，要求具有以下属性：](#qsck-1711641049261)

<a name="4f6p-1741350287258"></a>[唯一性：没有两个屏蔽令牌请求返回相同的值](#tvxj-1711641079676)

<a name="uxos-1741350287260"></a>[单调序列：时序实现单调](#2d0a-1711641105012)

<a name="5ce3-1741350287262"></a>[可用性：请求防护令牌且不会崩溃的节点，最终会收到响应](#mlbd-1711641130527)

<a name="y1dj-1741350287264"></a>[安全性和活性](#1lz8-1711387585629)

<a name="ara5-1741350287266"></a>[澄清这种情况，需区分两种不同性质](#hctm-1711641203049)

<a name="umik-1741350287268"></a>[安全性safety: 唯一性和单调序列](#k48b-1711641295367)

<a name="ldiu-1741350287270"></a>[活性：可用性](#fmy7-1711641313609)

<a name="7kfz-1741350287272"></a>[将系统模型映射到现实世界](#ndiw-1711387592922)

<a name="jrue-1741350287274"></a>[9.一致性共识](#rp9s-1705080859004)

<a name="ihan-1741350287276"></a>[共识consensus: 让所有节点对某件事达成一致](#vrvh-1712996687843)

<a name="3j6t-1741350287278"></a>[脑裂brain split: 两个节点都认为自己是master, 经常导致数据丢失；](#oqcs-1712996726634)

<a name="4quv-1741350287280"></a>[一致性保证](#obqn-1711730044966)

<a name="zxj3-1741350287282"></a>[最终一致性：收敛convergence, 最终收敛到相同的值](#ku5g-1712996774138)

<a name="x9ow-1741350287284"></a>[事务一致性：避免同时执行事务导致的竞争状态](#ga60-1712996859001)

<a name="qw57-1741350287286"></a>[分布式一致性模型：面对延迟和故障时，如何协调副本间的状态](#eovi-1712996916251)

<a name="mery-1741350287288"></a>[线性一致性](#thyh-1711730049653)

<a name="ufd4-1741350287290"></a>[背后的想法：原子一致性atomic consistency, 强一致性strong consistency, 立即一致性 immediate consistency ,外部一致性 external consistency](#bvre-1712997165569)

<a name="jr2v-1741350287292"></a>[看起来只有一个数据副本，实际有多个副本 - 强一致性就是逆分布式化，看成一个整体。](#qbsw-1712997247742)

<a name="y9t8-1741350287294"></a>[线性一致是一个新鲜度保证 recency gurantee;](#f0su-1712997377059)

<a name="ttym-1741350287296"></a>[非线性一致的例子： world cup ,  alice refresh - DB1 return germany 1: 0 argentina won ;  bob refresh website - DB2(un asychronize) return still playing;](#mtb7-1712997407717)

<a name="wgiz-1741350287298"></a>[为什么使得系统线性一致？](#qimr-1711730403998)

<a name="lbbq-1741350287300"></a>[例子：CAS使得多个客户端并发按序更新](#ffat-1712998155207)

<a name="acg6-1741350287302"></a>[线性一致性与可序列化是两种完全不同的保证：](#3elk-1712997586736)

<a name="y2hq-1741350287304"></a>[可序列化serializeble: 单对象或者多对象操作，确保按照某种顺序执行](#qpvu-1712998010030)

<a name="r3o6-1741350287306"></a>[线性一致性Linearizability:是读取和写入寄存器（单个对象）的新鲜度保证，不会将操作组合为事务，不会阻止写入偏差等问题](#ojsh-1712998125336)

<a name="ixpb-1741350287308"></a>[单副本强可串行性strong-1SR: 一个数据库可提供可串行性和线性一致, 基于两阶段锁2PL和实际串行执行通常是线性一致的](#5hda-1712998332148)

<a name="sx6z-1741350287310"></a>[SSI一致性快照不是线性一致的：不会包括比快照更新的写入；-不会阻塞非写的读。](#obfn-1712998432352)

<a name="wokd-1741350287312"></a>[依赖线性一致性](#bje8-1711730427157)

<a name="s9p5-1741350287314"></a>[锁定与领导选举](#xyll-1711730652620)

<a name="hzne-1741350287316"></a>[领导选举的一种方法：每个节点启动时尝试获取锁，成功者称为领导者。](#2fqs-1712998599719)

<a name="7sa3-1741350287318"></a>[保证锁时线性一致的： zookeeper 和 etcd 协调服务的一致性算法。curator基于zookeeper上更高级别的配方](#cbzr-1712998723133)

<a name="qgq9-1741350287320"></a>[约束和唯一性保证](#gpva-1711730659968)

<a name="bc1o-1741350287322"></a>[这种情况类似于一个锁，锁定用户名。该操作与原子性的cas类似：](#9jmj-1712998930168)

<a name="eyf5-1741350287324"></a>[用户名和邮箱；](#lhyk-1712999021176)

<a name="0ift-1741350287326"></a>[银行账号不为负数；](#7sgo-1712999031257)

<a name="nopv-1741350287328"></a>[航班剧院预定了同一时间同一位置](#so5k-1712999045093)

<a name="sxjx-1741350287330"></a>[实际应用中，处理这些限制有时可以接受。如调换位置并补偿。](#q05j-1712999070223)

<a name="5hre-1741350287332"></a>[跨信道的时序依赖](#7cbv-1711730671963)

<a name="sopl-1741350287334"></a>[将用户上传的照片降低分辨率以加快下载速度：](#cqkp-1712999245541)

<a name="izyd-1741350287336"></a>[步骤：消息队列一般针对较短的消息而设计，一张图片可能达到几M.所以步骤为](#3zoo-1712999362806)

<a name="5jpt-1741350287338"></a>[1.上传照片-  web server-2.  (store img) 存储文件服务器  ](#umkb-1712999318589)

<a name="dqti-1741350287340"></a>[  | 3. send message  || 5.fetch full size img  6.store resized img](#er8u-1712999518585)

<a name="jsri-1741350287342"></a>[消息队列   - 4.deliver message     image resizer](#iiv4-1712999532781)

<a name="wxmp-1741350287344"></a>[问题：存储和缩放存在两个不同的信道，没有线性一致的新鲜度保证](#0yys-1712999647965)

<a name="qxrb-1741350287346"></a>[3和4可能比存储服务器内部的复制更快，当5时，可能会看到图像旧版本或者什么都没有，如果处理的是旧版本，则产生不一致。](#xtun-1712999768959)

<a name="0eok-1741350287348"></a>[实现线性一致性的系统](#ytt0-1711730433598)

<a name="lnmj-1741350287350"></a>[单主复制（可能线性一致）protential潜在线性一致：](#j997-1712999879559)

<a name="bd4e-1741350287352"></a>[快照隔离并发错误；](#t174-1712999979364)

<a name="ydnk-1741350287354"></a>[脑裂；异步复制延迟，故障切换](#hh6o-1712999996439)

<a name="4gsu-1741350287356"></a>[共识算法（线性一致）：共识算法可以安全实现线性一致性的存储，zookeeper 和 etcd](#z3nq-1713000050923)

<a name="fdl8-1741350287358"></a>[多主复制（非线性一致）：同时多个节点上处理写入，异步复制到其他节点，会产生冲突的写入](#q0kz-1713000066034)

<a name="tz8p-1741350287360"></a>[无主复制（也许不是线性一致的）：Dynamo风格，法定人数读写获得强一致性。通常不完全正确](#ca4m-1713030167882)

<a name="3zk7-1741350287362"></a>[线性一致性和法定人数](#9gst-1711730681011)

<a name="9vql-1741350287364"></a>[最安全的做法是：假设采用Dynamo风格的无主复制的系统是不能提供线性一致性。](#sqvx-1713030399799)

<a name="lg1i-1741350287366"></a>[线性一致性的代价](#vpxf-1711730440465)

<a name="ofky-1741350287368"></a>[CAP定理](#fqqf-1711730717308)

<a name="3ypy-1741350287370"></a>[不仅是单主多主复制的后果，任何线性一致性的数据库都有这个问题：](#8hzl-1713030564813)

<a name="hmzc-1741350287372"></a>[.应用需要线性一致性，副本因为网络问题断开连接，请求必须等到网络问题解决或直接返回错误（不可用unavailable）](#7lux-1713030592466)

<a name="7mev-1741350287374"></a>[.网络断开也可独立处理请求（如多主复制），可以在网络问题前保持可用，但其行为不是线性一致的。](#48wk-1713031150235)

<a name="zmzj-1741350287376"></a>[CAP定理没有帮助：网络分区是不可避免的，所以只能是AP 或 CP。](#lsdc-1713031249676)

<a name="xesu-1741350287378"></a>[线性一致性和网络延迟](#kxsm-1711730728177)

<a name="6sej-1741350287380"></a>[ 线性一致的系统惊人的少。例如多核CPU并没有保证一定能读到第一个线程写入的值（除非使用内存屏障memory barrier或围栏fence）](#lbl2-1713031508671)

<a name="ncay-1741350287382"></a>[ 牺牲线性一致性的原因是性能performance. 内存访问先走缓存，副本都是异步更新的，所以失去了线性一致性。](#pnsn-1713031585885)

<a name="681x-1741350287384"></a>[搞笑的线性一致性存储不可实现。](#8y6v-1713031685808)

<a name="xje6-1741350287386"></a>[顺序保证](#on5f-1711730057760)

<a name="vmoh-1741350287388"></a>[写入顺序order of write: 单主复制的主要目的就是复制日志中确定写入顺序。](#dhiv-1713031758875)

<a name="vjoe-1741350287390"></a>[序列顺序serial order：某种序列顺序some sequential order](#cyfr-1713031815356)

<a name="fqbl-1741350287392"></a>[顺序与因果](#z48k-1711730472679)

<a name="rxli-1741350287394"></a>[顺序有助于保持因果关系causality](#khzl-1713031939113)

<a name="noh3-1741350287396"></a>[一致前缀读：先看到答案再看到问题。违背因cause果effect. 存在因果依赖causal dependency](#nbes-1713031966982)

<a name="15lg-1741350287398"></a>[更新不存在的记录：3位领导者间的复制，一些写入会压到其他写入。一条记录需被创建才能更新。](#xehj-1713032060272)

<a name="f3ua-1741350287400"></a>[检测并发写入：A和B并发，则不知道顺序，A和B不知道彼此。](#v1kv-1713032135082)

<a name="tdla-1741350287402"></a>[快照隔离：写偏差read skew，读取的数据违反因果关系（不可重复读）](#ozi8-1713032226341)

<a name="dqvi-1741350287404"></a>[事务的写偏差write skew：幻读和值班。](#pgzq-1713032264605)

<a name="e9jb-1741350287406"></a>[世界杯球赛结果，跨信道的时序依赖。](#3ang-1713032756197)

<a name="hyog-1741350287408"></a>[因果一致causally：系统服从因果关系锁规定的顺序。](#tryg-1713032788961)

<a name="h4h1-1741350287410"></a>[因果顺序不是全序的](#niwx-1711730751513)

<a name="yjba-1741350287412"></a>[全序total order: 自然数更大或更小，总是有序的](#x8gk-1713086054855)

<a name="t4hx-1741350287414"></a>[偏序partially order: 一个集合大于另一个集合，但是其它情况下无法比较](#wvsa-1713086222914)

<a name="fdan-1741350287416"></a>[线性一致性：操作是全序的，系统表现好像只有一个副本，所有操作都是原子性的。意味对每个操作总能判定发生顺序](#u7jq-1713086312064)

<a name="p12w-1741350287418"></a>[因果性：两个操作没在彼此之前发生，即并发无法比较顺序，意味着因果关系定义了一个偏序，而不是全序。一些有序一些无序](#xa0w-1713086399916)

<a name="spib-1741350287420"></a>[线性一致的数据存储中是不存在并发操作的。确保每个请求都是在唯一时间线上自动处理](#uaxz-1713086531138)

<a name="lnoz-1741350287422"></a>[并发意味着时间线会分叉然后合并。操作的偏序：](#qs64-1713086635361)

<a name="88vf-1741350287424"></a>[类似git系统，一个提交commit发生在另一个提交之后。分支合并merge会在这些并发创建的提交相融合时创建。](#ctkd-1713086670170)

<a name="9ri3-1741350287426"></a>[线性一致性强于因果一致性](#qdnf-1711730766239)

<a name="ffyt-1741350287428"></a>[线性一致性隐含implies着因果关系;](#dam4-1713086279684)

<a name="ja1r-1741350287430"></a>[捕获因果关系](#uqqx-1711730776113)

<a name="rtow-1741350287432"></a>[为了维持因果性，需确认操作发生在哪个操作之前happened before:](#agrw-1713086796608)

<a name="vgjv-1741350287434"></a>[刑侦决定Y时是否知到X与Y因果关系](#rys3-1713086865582)

<a name="abf0-1741350287436"></a>[检测并发写入：推广版本向量解决因果依赖](#znup-1713086911030)

<a name="qt8h-1741350287438"></a>[SSI序列化隔离快照：事务提交时，检测读取是否最新，为此跟踪哪些事务被那些事务锁读取。](#1ivk-1713086936754)

<a name="xucy-1741350287440"></a>[序列号顺序](#jsuf-1711730485869)

<a name="4pcb-1741350287442"></a>[序列号sequence number或时间戳timestamp](#y9ti-1713091524670)

<a name="fgwg-1741350287444"></a>[非因果序列号生成器](#qpya-1711730796494)

<a name="pppe-1741350287446"></a>[兰伯特时间戳](#mtzj-1711730811106)

<a name="bf3s-1741350287448"></a>[兰伯特时间戳提供了与因果关系一致的总排序：施行一个全序](#f8uv-1713091653199)

<a name="v6av-1741350287450"></a>[检测并发写入有着不同目的：区分两个操作是并发的，一个因果依赖另一个。](#2ayx-1713091789731)

<a name="v9he-1741350287453"></a>[光有时间戳排序还不够](#elc6-1711730816628)

<a name="xpip-1741350287455"></a>[全序广播](#aw2l-1711730490423)

<a name="v0c9-1741350287457"></a>[可靠交付reliable delivery：没有消息丢失，如果消息被传递到一个节点，它将被传递到所有节点](#axul-1713091937508)

<a name="k93y-1741350287459"></a>[全序交付totally ordered delivery: 消息以相同的顺序传递给每个节点。](#cruc-1713091989510)

<a name="vwzp-1741350287461"></a>[使用全序广播](#hp3a-1711730834892)

<a name="qdrq-1741350287463"></a>[zookeeper和etcd共识服务实际上实现了全序广播](#ztws-1713108978601)

<a name="ja1v-1741350287465"></a>[状态机复制state machine repliacation: 每个副本都按相同的顺序处理相同的写入。](#i8bk-1713109007327)

<a name="2qz6-1741350287467"></a>[与之类似，使用全序广播来实现可序列化的事务：真的串行执行，确定的事务以存储过程来执行，每个节点以相同的顺序处理这些消息。](#8jch-1713109071990)

<a name="qs3o-1741350287469"></a>[以及防护令牌的锁服务，全序广播使得所有消息按顺序一次编号，序列号可当做防护令牌(单调递增)，zookeeper中这个序列号称为zxid;](#w19a-1713109204107)

<a name="albp-1741350287471"></a>[使用全序广播实现线性一致的存储](#i3vg-1711730844593)

<a name="wj5k-1741350287473"></a>[使用线性一致性存储实现全序广播](#xlae-1711730855140)

<a name="h9br-1741350287475"></a>[分布式事务与共识](#y1bv-1711730380233)

<a name="jvm0-1741350287477"></a>[让几个节点达成一致get serveral nodes to agree on somthing](#yt5r-1713109388776)

<a name="1uao-1741350287479"></a>[领导选举：避免脑裂](#d7t7-1713109479427)

<a name="nlii-1741350287481"></a>[原子提交atomic commit：多节点事务提交/中止/回滚](#frsp-1713109508353)

<a name="2wpy-1741350287483"></a>[原子提交与两阶段提交2PC](#abk6-1711730509647)

<a name="hsji-1741350287485"></a>[原子性可以避免中间态，多对象事务原子性确保二级索引与主数据库保持一致。](#0h2f-1713110484428)

<a name="bymo-1741350287487"></a>[从单节点到分布式原子提交](#iqfd-1711730882368)

<a name="hfim-1741350287489"></a>[从数据库的角度看，这是单独的事务，任何跨事务正确性的保证都是应用自己的问题。](#h1ym-1713110537245)

<a name="q0va-1741350287491"></a>[2PC两阶段提交简介](#roa9-1711730906391)

<a name="rw0k-1741350287493"></a>[coodinator协调子事务，子事务单独执行完，在准备阶段询问是否可提交](#vaax-1705118249954)

<a name="0qzh-1741350287495"></a>[.全部为是，则分别提交。](#7zwc-1705118288968)

<a name="rjs7-1741350287497"></a>[.其中有否，则中断回滚。](#3b8s-1705118337513)

<a name="tktw-1741350287499"></a>[系统承诺](#kpgk-1711730913393)

<a name="wlsg-1741350287501"></a>[一旦协调者做出决定，这一决定是不可撤销的。](#wkui-1713110739239)

<a name="iyjk-1741350287503"></a>[协调者失败](#qhdj-1711730921387)

<a name="zi1r-1741350287505"></a>[参与者投赞成票后，参与者协调者崩溃，数据库1不知道是否提交或中止。](#jpd8-1713110804532)

<a name="dv4c-1741350287507"></a>[三阶段提交](#ixzv-1711730924556)

<a name="otvd-1741350287509"></a>[阻塞blocking原子提交协议, 2PC可能卡主等待协调者恢复的情况。](#edp3-1713111179295)

<a name="gcaj-1741350287511"></a>[在提交阶段，如果子事务进程crash,或者协调者崩溃的时的解决方案，弱原子性，最终一致性。](#llyc-1711731148564)

<a name="nk9l-1741350287513"></a>[许多coodinator不支持高可用或者只支持基本的复制](#nlrk-1705118500709)

<a name="dp3p-1741350287515"></a>[实践中的分布式事务](#xexf-1711730524691)

<a name="l2h7-1741350287517"></a>[云服务由于其导致的运维问题而选择不实现分布式事务。](#inqh-1713111548114)

<a name="tu4z-1741350287519"></a>[两阶段难以实现安全保证，且导致的运维问题造成性能下降。](#zuho-1713111590325)

<a name="sgag-1741350287521"></a>[数据库内部的分布式事务](#gf92-1711731287296)

<a name="50dp-1741350287523"></a>[VoltDB 与mysql Cluster 的NDB引擎就有这样的内部事务支持。](#jhqw-1705080950920)

<a name="wxu9-1741350287525"></a>[异构分布式事务](#gvng-1713111648038)

<a name="w4og-1741350287527"></a>[保证异构数据库的事务更有挑战性](#st60-1713111659854)

<a name="ri16-1741350287529"></a>[恰好一次的消息处理](#btdt-1711730944613)

<a name="stgj-1741350287531"></a>[XA事务eXtended Architecture](#qvbk-1711730954739)

<a name="8ut4-1741350287533"></a>[怀疑时持有锁](#med0-1711730959684)

<a name="lily-1741350287535"></a>[从协调者故障中恢复](#ouyq-1711730969007)

<a name="ygp1-1741350287537"></a>[唯一的出路是让管理员手动决定提交还是回滚事务](#ouur-1713116418144)

<a name="1n6t-1741350287539"></a>[分布式事务的限制](#gzya-1711730975747)

<a name="8wbn-1741350287541"></a>[协调者本身就是一种数据库，存储了事务的结果。](#jkt3-1713116480399)

<a name="axey-1741350287543"></a>[协调者默认不是高可用的，只有基本的复制](#mcvv-1713116529836)

<a name="jdcg-1741350287545"></a>[应用服务器不再是无状态的了](#h6gt-1713116550997)

<a name="9lwu-1741350287547"></a>[不能检测不同系统间的死锁，且无法与SSI协同工作](#rrrl-1713116568769)

<a name="mohl-1741350287549"></a>[与构建容错系统的目标背道而驰，分布式事务有扩大失效amplifying failures的趋势](#wbzw-1713116600081)

<a name="ag5v-1741350287551"></a>[容错共识](#npne-1713118985168)

<a name="cqqb-1741350287553"></a>[一个或多个节点提议propose某些值，共识算法决定decides采用其中的某个值。](#beiq-1713118985169)

<a name="yz0y-1741350287555"></a>[一致同意Uniform agreement:  没有两个节点的决定不同-核心 决定了相同的结果，一旦决定不能改变](#pq0x-1713118985170)

<a name="qx3o-1741350287557"></a>[完整性Integrity: 没有节点决定两次-核心](#xnwp-1713118985171)

<a name="heq6-1741350287559"></a>[有效性validity: 如果一个节点决定了值v, 则v由某个节点所提议-排除平凡的解决方案](#2bty-1713118985172)

<a name="ndk0-1741350287561"></a>[终止Termination由所有为崩溃的几点来最终决定值。-活性属性，必须取得进展，即使部分节点出现故障，其他节点也必须达成一致。不超过一般的节点崩溃或不可达](#9k32-1713118985173)

<a name="fbh8-1741350287563"></a>[稳健达成共识：只要少于三分之一的节点存在拜占庭故障](#scne-1713118985174)

<a name="i8xt-1741350287565"></a>[共识算法和全序广播](#prxr-1713118985175)

<a name="4wyg-1741350287567"></a>[视图戳复制VSR viewstamped repliacation, Paxos，Raft, Zab](#zmcg-1713118985176)

<a name="q07x-1741350287569"></a>[全序广播相当于重复进行多轮共识：](#1u8f-1713118985177)

<a name="paz1-1741350287571"></a>[一致同意属性，所有节点决定以相同的顺序传递相同的消息](#d2l2-1713118985178)

<a name="ftl2-1741350287573"></a>[完整性属性，消息不会重复](#z7u4-1713118985179)

<a name="1e94-1741350287575"></a>[有效性属性，消息不会被破坏，也不能凭空编造](#pphb-1713118985180)

<a name="37hb-1741350287577"></a>[终止属性,消息不会丢失](#gqmq-1713118985181)

<a name="jwvz-1741350287579"></a>[视图戳复制，raft和zab直接实现了全序广播，比重复一次一致one value a time的共识更高效， paxos 下这种优化称为multi-paxos.](#lxll-1713118985182)

<a name="gsbu-1741350287581"></a>[单领导者复制和共识](#krsz-1713118985183)

<a name="gvwn-1741350287583"></a>[时代编号和法定人数](#90tt-1713118985184)

<a name="sepy-1741350287585"></a>[共识的局限性](#qywj-1713118985185)

<a name="nfyx-1741350287587"></a>[成员与协调服务](#8qzt-1713118985186)

<a name="aovz-1741350287589"></a>[将工作分配给节点](#4tlb-1713118985187)

<a name="lbms-1741350287591"></a>[服务发现](#tbbj-1713118985188)

<a name="j00d-1741350287593"></a>[成员服务](#1lld-1713118985189)

<a name="5jaf-1741350287595"></a>[第三部分：衍生数据](#46k8-1705118453932)

<a name="kvdg-1741350287597"></a>[记录和衍生数据系统](#rboc-1716226529015)

<a name="oybd-1741350287599"></a>[从高层次看，存储和处理数据的系统可以分为两大类](#cmdh-1716226737947)

<a name="8toy-1741350287601"></a>[记录系统System of record](#1dmv-1716226552894)

<a name="fnlj-1741350287603"></a>[真相源source of truth. 记录用户输入新数据，标准化normalized](#dh96-1716226777418)

<a name="rju8-1741350287605"></a>[衍生数据系统Derived data system](#qjlf-1716226568395)

<a name="y8yr-1741350287607"></a>[缓存cache, 衍生数据是冗余的，重复了已有的信息。](#igtl-1716226871151)

<a name="kmnb-1741350287609"></a>[10.批处理](#0bgb-1716226496297)

<a name="i7zx-1741350287611"></a>[服务（在线系统）：高可用性，性能指标-响应时间](#yygb-1720542683707)

<a name="dq13-1741350287613"></a>[批处理系统（离线系统）：定时任务job，性能指标-吞吐量](#goo7-1720542700859)

<a name="nsml-1741350287615"></a>[流处理系统（准实时系统）：介于在线离线系统之间](#mv2a-1720542952432)

<a name="iotz-1741350287617"></a>[使用Unix工具的批处理](#6lu4-1716228538928)

<a name="nyjt-1741350287619"></a>[分析简单日志](#hyii-1716228597300)

<a name="zvsf-1741350287621"></a>[Unix命令链：网站上找到五个最受欢迎的网页（nginx/access.log）](#smoq-1720543218731)

<a name="oycn-1741350287623"></a>[命令链与自定义程序Ruby](#ukyi-1716228816049)

<a name="vafs-1741350287625"></a>[排序 VS 内存中的聚合](#cnuw-1716228829632)

<a name="k492-1741350287627"></a>[工作集working set(作业需要随机访问的内存大小)，单个url 重复出现百万次，哈希表性能好](#siiu-1720543940755)

<a name="ylhr-1741350287629"></a>[工作集大于内存，排序优，与SSTables 和 LSM树一样，内存聚合归并排序，GNU 中的sort通过溢出至磁盘自动应该大于内存大额数据集，瓶颈可能是磁盘io。-MQ的死信处理也是溢出到磁盘。](#lms0-1720544023798)

<a name="wlyg-1741350287631"></a>[Unix哲学](#mgei-1716228613042)

<a name="0vtw-1741350287633"></a>[统一的接口](#sk3b-1716228852580)

<a name="2bci-1741350287635"></a>[Unix中统一的接口是一个文件file](#33ju-1720874904091)

<a name="q3eq-1741350287637"></a>[ASCII文本，\n 分隔，](#yogq-1720875513301)

<a name="exck-1741350287639"></a>[CSV， ，号分隔](#7fvb-1720875542091)

<a name="cnki-1741350287641"></a>[逻辑与布线相分离](#3oxq-1716228869387)

<a name="xcna-1741350287643"></a>[松耦合loose coupling](#0zy9-1720876020643)

<a name="eqng-1741350287645"></a>[晚期绑定late binding](#226j-1720876035472)

<a name="78hz-1741350287647"></a>[控制反转inversion of control](#zedb-1720876065554)

<a name="qgxj-1741350287649"></a>[透明度与实验](#ftj8-1716228879278)

<a name="goxl-1741350287651"></a>[MapReduce和分布式文件系统](#djdy-1716228554334)

<a name="jqe3-1741350287653"></a>[MapReduce像是分布在数前台机器上的Unix。](#illh-1720876369554)

<a name="kl6r-1741350287655"></a>[Hadoop的Map-Reduce视线中，文件系统成为HDFS,基于google文件系统GFS的开源实现。](#e1cy-1720876447842)

<a name="5pnf-1741350287657"></a>[Amazon S3，  Azure blob ](#mapk-1720876955975)

<a name="0urq-1741350287659"></a>[MapReduce作业执行](#kdge-1716228625984)

<a name="lnvm-1741350287661"></a>[Mapper在每条输入记录上调用一次,从输入记录中提取键值（提取键值）](#vleh-1720877349422)

<a name="myko-1741350287663"></a>[Reduce拉取Mapper生成的键值对，收集属于同一个键的所有值。（收集同键的所有值）](#aol7-1720877397533)

<a name="h1ca-1741350287665"></a>[分布式执行MapReduce](#kv6u-1716231603620)

<a name="wba4-1741350287667"></a>[即在不同服务器上执行mapper reduce 的task。](#wy7r-1720877483243)

<a name="46hm-1741350287669"></a>[MapReduce工作流](#d0qe-1716231619319)

<a name="nduo-1741350287671"></a>[workflow。一系列命令，输出写入临时文件，下一个命令从临时文件中读取-物化中间状态](#rvme-1720877910897)

<a name="e1ow-1741350287673"></a>[Reduce端连接与分组](#pyoc-1716228643855)

<a name="l462-1741350287675"></a>[用户活动事件](#1pza-1716231634942)

<a name="a6zy-1741350287677"></a>[左侧是事件日志/活动事件activity events/点击流事件clickstream evnets](#4l72-1720878002631)

<a name="hiql-1741350287679"></a>[右侧是用户数据库](#hivt-1720878131008)

<a name="mqom-1741350287681"></a>[用户行为日志与用户档案的连接](#cvii-1720878237531)

<a name="x667-1741350287683"></a>[原始:逐个遍历活动事件关联查询。缺点：吞吐量受限数据库服务器且容易压垮](#eiqe-1720878248508)

<a name="3x3h-1741350287685"></a>[改良：获取用户数据库副本（ETL进程从数据库备份中提取数据，参考数仓），放在同一个分布式文件系统中。将用户数据库存储HDFS中的一组文件中，用户活动记录存储另一组文件中，](#iueg-1720878322010)

<a name="xklz-1741350287687"></a>[并能用MapReduce将左右相关记录集中到同一个地方进行高效处理。](#ruua-1720878498786)

<a name="qtmx-1741350287689"></a>[排序合并连接](#fihy-1716231663765)

<a name="q04i-1741350287691"></a>[1.map提取键值(userid-url  usrid-do)和reduce(userid all url do)](#q51q-1720947527716)

<a name="iqy5-1741350287693"></a>[2.二次排序secondary sort ：对1的合并按时间戳排序](#4kvv-1720947648039)

<a name="rhzb-1741350287695"></a>[排序合并连接sort-merge join: reduce一次处理一个特定用户所有记录，只需将一条用户记录保存在内存中，无需发送网络请求](#j8br-1720947805879)

<a name="kuk2-1741350287697"></a>[把相关数据放在一起](#8vrk-1716231670998)

<a name="kzu6-1741350287699"></a>[Mapper和排序过程确保特定用户id连接操作的必须数据都放在同一个地方](#t1tc-1720947947656)

<a name="elzc-1741350287701"></a>[GROUP BY](#9j4s-1716228658160)

<a name="y6a5-1741350287703"></a>[除了连接之外，“把相关数据放在一起”的常见模式是记录分组。SQL中的Group By](#a6wg-1720952083180)

<a name="ycya-1741350287705"></a>[处理倾斜](#qa24-1716231688547)

<a name="dzah-1741350287707"></a>[名流热点数据hot spot](#f55j-1720952155251)

<a name="czyu-1741350287709"></a>[参考负载倾斜和消除热点](#julg-1720952222343)

<a name="8ids-1741350287711"></a>[Mapper将热键的关联记录随机发送到几个Reduce之一](#akfb-1720952286891)

<a name="hura-1741350287713"></a>[Map端连接](#2tox-1716228671225)

<a name="q73w-1741350287715"></a>[预处理输入数据](#qiil-1720971178915)

<a name="llcd-1741350287717"></a>[广播散列连接](#wjhj-1716231713037)

<a name="440m-1741350287719"></a>[broadcast hash join](#k3se-1720971260954)

<a name="qr3o-1741350287721"></a>[分区散列连接](#su27-1716231723170)

<a name="v7wq-1741350287723"></a>[Map桶端连接bucketd map jons](#5h9a-1720971514797)

<a name="fiw3-1741350287725"></a>[Map端合并连接](#ftdq-1716231736611)

<a name="l8ca-1741350287727"></a>[MapReduce工作流与Map端连接](#yrmc-1716231744413)

<a name="5dzl-1741350287729"></a>[批处理工作流的输入](#zzir-1716228678980)

<a name="rmck-1741350287731"></a>[OLTP ：查找少量记录，使用索引，呈现给用户](#hiqn-1720971586510)

<a name="u3ts-1741350287733"></a>[OLAP：扫描大量记录，执行分组与聚合，输出有着报告的形式，商业决策的分析。](#cqba-1720971612883)

<a name="l4iq-1741350287735"></a>[建立搜索索引](#bwjf-1716231761454)

<a name="0uwj-1741350287737"></a>[MapReduce最初作为google搜索引擎的索引](#lypn-1720971694941)

<a name="mtko-1741350287739"></a>[全文搜索和模糊索引  Lucene](#iijq-1720971776882)

<a name="o47d-1741350287741"></a>[分区和二级索引](#vcun-1720971828838)

<a name="2qop-1741350287743"></a>[键值存储作为批处理输入](#8fix-1716231782091)

<a name="wjga-1741350287745"></a>[批处理输入的哲学](#audd-1716231802005)

<a name="9ars-1741350287747"></a>[视为不可变且避免副作用：](#6i34-1720972317458)

<a name="pp6g-1741350287749"></a>[人类容错human fault tolerance](#zciu-1720972319614)

<a name="hmb1-1741350287751"></a>[最小化不可逆性mininizing irreversibility](#oolm-1720972319615)

<a name="mpw9-1741350287753"></a>[Hadoop与分布式数据库的对比](#lp33-1716228684462)

<a name="ojdd-1741350287755"></a>[Hadoop 像Unix的分布式版本，HDFS是它的文件系统，而MapReduce是Unix进程的怪异实现](#g0vj-1720972341966)

<a name="bc8v-1741350287757"></a>[MapReduce论文发表前，并行连接算法已在数据库实现-大规模并行处理MPP massively parallel processing ](#gjbt-1720972427948)

<a name="pnkj-1741350287759"></a>[Gamma database machine, Teradata, Tandem NonStop SQL](#go7m-1720972523351)

<a name="u0r7-1741350287761"></a>[区别：MMP专注一组及骑行并行执行分析SQL 查询](#5mnr-1720972551600)

<a name="neme-1741350287763"></a>[MapReduce和分布式文件系统的组合是一个可以运行任意程序的通用操作系统](#gx7b-1720972580058)

<a name="umcj-1741350287765"></a>[存储多样性](#eebl-1716231817367)

<a name="xw6q-1741350287767"></a>[数据湖data lake / 企业数据中心enterprise data hub](#5lga-1720972673599)

<a name="ez5x-1741350287769"></a>[Hadoop 常用于实现数仓ETL Extract, Transform, Load](#jrhw-1720972777529)

<a name="jlqo-1741350287771"></a>[处理模型多样](#ysfv-1716231826972)

<a name="sec1-1741350287773"></a>[针对频繁故障设计](#kwz4-1716231831752)

<a name="0lqf-1741350287775"></a>[MapReduce之后](#p0zx-1716228568687)

<a name="cvz0-1741350287777"></a>[建立在MapReduce上的高级编程模型](#wcyn-1721117329960)

<a name="qqeg-1741350287779"></a>[Pig，Hive, Cascading, Crunch](#l9ug-1721117359714)

<a name="nq9m-1741350287781"></a>[流处理：加速批处理的另一种方法](#bz2s-1721117576614)

<a name="v9wf-1741350287783"></a>[物化中间状态](#fvak-1716228706372)

<a name="dn8h-1741350287785"></a>[中间状态intermediate state](#npb7-1721117756510)

<a name="alht-1741350287787"></a>[物化materialization:中间态写入文件的过程](#0rlu-1721117876651)

<a name="1vsz-1741350287789"></a>[MapReduce完全物化中间态的方法不足：](#fnqf-1721117967688)

<a name="j4my-1741350287791"></a>[只有在前驱作业生成其输入完成时才能启动](#pood-1721117984973)

<a name="uky2-1741350287793"></a>[mapper通常是多余的：仅仅读取由Reducer写入的同样文件](#iljf-1721118032959)

<a name="j9t8-1741350287795"></a>[临时文件（数据）被复制到多个节点](#tghz-1721118071785)

<a name="r4eu-1741350287797"></a>[解决：数据流引擎](#aer2-1721118148615)

<a name="hhqg-1741350287799"></a>[数据流引擎](#sdac-1716231877023)

<a name="wku4-1741350287801"></a>[分布式批处理新引擎：](#4gu5-1721118111495)

<a name="jxi2-1741350287803"></a>[spark,  Flink: 独立的网络通信层，调度器，用户向API](#nfnv-1721118162692)

<a name="18wl-1741350287805"></a>[Tez : 薄库，依赖YARN shuffle服务实现节点间的数据复制](#0kbz-1721123564974)

<a name="lnl2-1741350287807"></a>[dataflow engines模型:](#jojo-1721118227522)

<a name="604y-1741350287809"></a>[函数/算子operators: 一个算子的输出连接到另一个算子的输入](#kstp-1721118246090)

<a name="q19y-1741350287811"></a>[优点：](#zbrh-1721118327524)

<a name="g7si-1741350287813"></a>[排序等昂贵工作只需在实际需要地方执行，不是默认在Map和Reduce阶段间](#8oqo-1721118330975)

<a name="msra-1741350287815"></a>[没有不必要的Map任务](#amzf-1721118375240)

<a name="9zar-1741350287817"></a>[连接和数据依赖都是显示声明的](#bunw-1721118382779)

<a name="8ykq-1741350287819"></a>[算子中间态足以保存内存或写入本地磁盘，比HDFS需要更少I/O](#bzui-1721122836028)

<a name="abjo-1741350287821"></a>[算子可在输入就绪后立即执行，不许等待前驱完成](#ka7f-1721123164712)

<a name="syen-1741350287823"></a>[MapReduce为每个任务启动新JVM, 而df enginer可重用现有JVM，减少启动开销](#rmr3-1721123191298)

<a name="bya4-1741350287825"></a>[执行速度更快](#mp4i-1721123431562)

<a name="cazt-1741350287827"></a>[使用：](#2cxo-1721123304059)

<a name="60ow-1741350287829"></a>[算子是Map和Reduce的泛化/抽象，相同处理代码可在任意引擎上运行](#vyfa-1721123442398)

<a name="eanp-1741350287831"></a>[Pig Hive Casacding中实现的工作流无需修改代码，修改配置，从MapReduce切换到Tez 或spark.](#qxck-1721123483308)

<a name="yscj-1741350287833"></a>[容错](#glfr-1716231882229)

<a name="u7vq-1741350287835"></a>[重启，算子重新计算](#neqa-1721499456929)

<a name="piof-1741350287837"></a>[关于物化的讨论](#npsg-1716231884238)

<a name="gcri-1741350287839"></a>[图与迭代处理](#dvkh-1716228727874)

<a name="wmv1-1741350287841"></a>[图数据模型常用于机器学习应用](#y476-1721499725472)

<a name="xsji-1741350287843"></a>[图形算法PageRank:根据连接到某个网页的其他网页来估计该网页的流行度](#ld62-1721499760165)

<a name="v1d3-1741350287845"></a>[闭包传递transitive clousure: 重复跟进标明地点归属关系的边，生成数据库中北美地点列表](#et5r-1721499798288)

<a name="gjx2-1741350287847"></a>[MapReduce低效：没考虑算法迭代性](#x4m1-1721499995671)

<a name="xmwf-1741350287849"></a>[Pregel处理模型](#lf5y-1716231892007)

<a name="ra0y-1741350287851"></a>[图批处理的优化：批量同步并行（BSP）计算模型。](#1faf-1721499959178)

<a name="8v9u-1741350287853"></a>[Pregel模型：Google的Pregel论文推广了处理图的方法。Apache Giraph , Spark GraphX API 和 Flink Gelly API](#3mh8-1721500149918)

<a name="lntl-1741350287855"></a>[容错](#pxnd-1716231909468)

<a name="j5q1-1741350287857"></a>[并行执行](#v8gr-1716231912877)

<a name="gxlp-1741350287859"></a>[高级API和语言](#cxqk-1716228736349)

<a name="p0q4-1741350287861"></a>[Hive,pig, Cascading,Crunch等高级语言和API, Tez](#qlnr-1721841646688)

<a name="r0ui-1741350287863"></a>[向声明式查询语言的转变](#dfa6-1716231923208)

<a name="eqmh-1741350287865"></a>[声明式declarative](#zman-1721841903902)

<a name="ez12-1741350287867"></a>[与SQL的完全声明式查询有很大的区别：](#inea-1721841971310)

<a name="mzpq-1741350287869"></a>[围绕回调函数的概念建立，自由调用任意代码来决定输出什么。](#ahki-1721842021736)

<a name="b5e5-1741350287871"></a>[可基于大量已有库的生态系统创作： 解析、自然语言分析，图像分析以及运算数值算法或统计算法等。](#bdyf-1721842047384)

<a name="zysf-1741350287873"></a>[专业化的不同领域](#m2pu-1716231937991)

<a name="8gq7-1741350287875"></a>[空间算法：最近邻搜索K-nearest neghbors](#wpot-1721842227412)

<a name="w1ya-1741350287877"></a>[11.流处理](#iwmv-1721931597264)

<a name="ldil-1741350287879"></a>[流处理Stream processing: 当事件发生时就可以立即处理](#yeid-1722103475505)

<a name="oph8-1741350287881"></a>[传递事件流](#08iq-1721844360659)

<a name="isvj-1741350287883"></a>[事件event：流处理的上下文件记录](#n0th-1722103658739)

<a name="me4f-1741350287885"></a>[生产者producer/发布者publisher/发送者sender](#titb-1722103707652)

<a name="vzfb-1741350287887"></a>[消费者consumer/订阅者subscribers/接收者recipients](#yhbb-1722103745716)

<a name="misa-1741350287889"></a>[主题topic/流stream：相关事件的聚合](#jdqp-1722103794393)

<a name="epvj-1741350287891"></a>[通知机制：数据库触发器trigger(算是弥补机制)、消息传递系统messaging system](#2buw-1722103875913)

<a name="ehsz-1741350287893"></a>[消息系统](#ipga-1721932434160)

<a name="3odj-1741350287895"></a>[两个关键问题：](#qm3o-1722104029796)

<a name="7wv2-1741350287897"></a>[1.生产者快于消费者：丢掉消息/将消息放入缓冲队列/背压backpressure(流量控制flow control) 阻塞生产者，以免其发送更多消息](#sztq-1722104046748)

<a name="2t00-1741350287899"></a>[2.节点崩溃或脱机，是否有消息丢失： 持久性，高吞吐和低延迟则接收丢失](#5ycq-1722104289674)

<a name="uz1c-1741350287901"></a>[直接从生产者传递给消费者](#cj2t-1721932544748)

<a name="lw2u-1741350287903"></a>[直接网络通信，而不通过中间节点：](#hjhp-1722104612024)

<a name="itmy-1741350287905"></a>[UPD组播，金融行业，低延时，不可靠，应用层协议可恢复丢失的包（生产者必须记住发送的包，按需重新发送）](#0ggy-1722104631748)

<a name="gegp-1741350287907"></a>[无代理的消息库，ZeroMQ、nanomsg, TCP或IP多播](#hnj8-1722104735615)

<a name="p3ug-1741350287909"></a>[StatsD 和 Brubeck 使用UDP](#pcd7-1722104800545)

<a name="qaqx-1741350287911"></a>[消费者在网络上公开了服务，生产者可以直接发送HTTP和RPC请求将消息推送给使用者webhooks.](#3se2-1722104863261)

<a name="mwhw-1741350287913"></a>[消息代理](#jwkh-1721932555905)

<a name="kj0p-1741350287915"></a>[消息代理message broker(也称为消息队列message queue)](#dtlb-1722106424439)

<a name="ygyz-1741350287917"></a>[消息代理与数据库对比](#fyus-1721932559957)

<a name="rwqa-1741350287919"></a>[.数据存储：数据库保留数据直至显示删除；大多数消息代理在消息成功传递给消费者时地总删除](#aj3x-1722106519844)

<a name="yhk9-1741350287921"></a>[.删除消息快，所以队列可以很短。如过缓冲很多消息可能会溢出到磁盘，吞吐量会下降](#pkoh-1722106591225)

<a name="bipt-1741350287923"></a>[.匹配方式：数据库通常支持二级索引和各种搜索数据的方式；消息代理按照模式匹配订阅子集](#lfst-1722106688956)

<a name="ynsr-1741350287925"></a>[.最新数据：数据库通常支持事务，基于某个时间点数据快照；消息代理不支持任意查询，有新消息会通知客户端](#q13l-1722106755764)

<a name="vr6d-1741350287927"></a>[多个消费者](#v0lh-1721932568637)

<a name="7bfk-1741350287929"></a>[负载均衡load balance: 多个客户端从同一个队列中消费。JMS中称为共享订阅shared subscripiton](#enlv-1722191268259)

<a name="biqx-1741350287931"></a>[扇出fan-out:将每条消息传递给多个消费者](#bjqd-1722191397355)

<a name="luxc-1741350287933"></a>[确认与重新交付](#aoas-1721932573289)

<a name="visq-1741350287935"></a>[消息代理使用确认acknowlegements:客户端需显示告知代理消息处理完毕的时间，代理将消息从队列中移除。](#8lsc-1723155930519)

<a name="99hw-1741350287937"></a>[确认丢失：需一种原子提交协议才能处理，如分布式事务。](#w6ac-1723156157534)

<a name="7sxe-1741350287939"></a>[负载均衡与重传的组合不可避免导致消息顺序(JMS，AMQP标准要求保留消息的顺序。)重新排序](#zlnz-1723156224601)

<a name="nojr-1741350287941"></a>[解决：每个消费者使用单独的队列-即不适用负载均衡功能](#fovj-1725028626326)

<a name="vuqf-1741350287943"></a>[消息完全独立，消息重排序并不是个问题，消息之间存在因果依赖关系则是个重要问题](#3kzh-1725028667923)

<a name="tvtt-1741350287945"></a>[分区日志](#9bmj-1721932438668)

<a name="ppb0-1741350287947"></a>[消息传递/不留痕迹：网络数据包 和 消息代理 (可抓包和日志持久)](#xree-1725028818863)

<a name="vpgd-1741350287949"></a>[持久化：数据库和文件系统，显式删除前将永久记录](#sbbo-1725028858585)

<a name="ahjh-1741350287951"></a>[消息代理log-base message brokers: 既有数据库的持久化，又有消息传递的低延迟通知。](#n6lq-1725028981222)

<a name="gmbf-1741350287953"></a>[使用日志进行消息存储](#keft-1721932590686)

<a name="w76t-1741350287955"></a>[生产者通过将消息追加到日志末尾来发送消息，消费者通过以此读取日志来接收消息。](#tlnm-1725029031868)

<a name="js1a-1741350287957"></a>[Unix tail -f 监视文件被追加写入的数据就是这样工作的。](#1606-1725029123561)

<a name="8akl-1741350287959"></a>[Apache Kafka, Amazon Kinesis Streams , Twitter DistributedLog都是基于日志的消息代理](#aq8x-1725029525384)

<a name="sc6c-1741350287961"></a>[Google Cloud Pub/Sub在架构上类似，单对外暴露的是JMS风格的API，而不是日志抽象](#0wz5-1725029598472)

<a name="wwok-1741350287963"></a>[尽管写入磁盘，但通过跨多台机器分区，每秒能实现数百万消息的吞吐量，并通过复制来实现容错性。](#zqkl-1725029648581)

<a name="4j95-1741350287965"></a>[日志与传统消息相比](#pr6y-1721932607757)

<a name="2ikd-1741350287967"></a>[天然支持扇出式消息传递：过个消费者独立读取日志，而不相互影响](#i3q5-1725115574888)

<a name="vovq-1741350287969"></a>[负载平衡：读取消息不会从日志中删除，代理可以将整个分区分配给消费者组中的节点，而不是单挑消息分配给客户端。](#ymqj-1725115609372)

<a name="idp9-1741350287971"></a>[消费者偏移量](#o0ft-1721932621047)

<a name="jnzi-1741350287974"></a>[和单领导者数据库复制中常见的日志序列号非常相似，代理无需跟踪每条消息，只需定期记录消费者偏移即可。有助于提高基于日志的系统吞吐量](#emow-1725115793168)

<a name="fdxx-1741350287976"></a>[问题：消费者节点失效而没有记录偏移量，重启后这些消息将被处理两次。本章后续讨论。](#gmal-1725115911246)

<a name="j1ss-1741350287978"></a>[磁盘空间使用](#ipmx-1721932629330)

<a name="os9v-1741350287980"></a>[只追加写入日志，回收时将旧段删除或移动到归档存储](#5ep6-1725115987947)

<a name="v3vb-1741350287982"></a>[问题，消费者跟不上生产者，指向删除段，错过一些消息。](#kjgm-1725116313754)

<a name="ztdf-1741350287984"></a>[日志实现一个有限大小的缓冲区：循环缓冲区circular buffer/环形缓冲区，填满时将丢弃旧消息](#k3hi-1725116359622)

<a name="c2ym-1741350287986"></a>[设 大型硬盘容量为 6TB, 顺序写入为150MB/s， 最快需要约11小时填满磁盘，之后开始覆盖旧消息。实践中的部署很少能用满磁盘的写入带宽。所以通常可保留一个几天甚至几周的日志缓冲区](#jjma-1725116440115)

<a name="o44r-1741350287988"></a>[以上保留多长时间的消息，日志吞吐量或多或少保持不变，因为每个消息都写入磁盘。](#bdoa-1725116521841)

<a name="lt1g-1741350287990"></a>[   而默认将消息保存在内存中，当队列很短，系统很快；当系统开始写入磁盘，就慢得多，所以吞吐量取决于保留的历史数量。](#rxx7-1725116637183)

<a name="npoc-1741350287992"></a>[当消费者更不上生产者时](#91b6-1721932634947)

<a name="dji1-1741350287994"></a>[3种选择：丢弃信息、进行缓冲或施加背压](#dztq-1725116764093)

<a name="ojke-1741350287996"></a>[巨大运维优势：丢失消息仅影响当前消费者，不会中断其它消费者，可进行调试测试，不必中断生产服务。](#mgyh-1726767519795)

<a name="xsz6-1741350287998"></a>[重播旧信息](#6tnb-1721932645175)

<a name="ssgy-1741350288000"></a>[删除消息：AMQP和JMS消息代理](#yygs-1726767645074)

<a name="vp0a-1741350288002"></a>[只读操作：基于日志的代理。](#u81b-1726767864887)

<a name="1ht9-1741350288004"></a>[更像上一章节的批处理。允许更多的实验操作，通过偏移量重播。](#opy5-1726767877078)

<a name="cx6b-1741350288006"></a>[流与数据库](#rnwj-1721932350699)

<a name="eix1-1741350288008"></a>[保持系统同步](#ehyq-1721932450135)

<a name="g4it-1741350288010"></a>[变更数据捕获](#1ako-1721932460819)

<a name="v0iz-1741350288012"></a>[变更数据捕获的实现](#xkpl-1721932670593)

<a name="6hbn-1741350288014"></a>[初始快照](#uxpn-1721932683846)

<a name="ertx-1741350288016"></a>[日志压缩](#j0w6-1721932690312)

<a name="guij-1741350288018"></a>[变更流的API支持](#5pqr-1721932694290)

<a name="0rlk-1741350288020"></a>[事件溯源](#qkmx-1721932472062)

<a name="ucss-1741350288022"></a>[从时间日志中派生出当前状态](#vydq-1721932714045)

<a name="vepn-1741350288024"></a>[命令和事件](#xbwr-1721932732154)

<a name="owaz-1741350288026"></a>[状态，流和不变形](#k3v8-1721932742549)

<a name="pfad-1741350288028"></a>[从同一事件日志中派生多个视图](#diql-1721932751200)

<a name="oq3k-1741350288030"></a>[并发控制](#tzcl-1721932772042)

<a name="caap-1741350288032"></a>[不变性的限制](#urmz-1721932775107)

<a name="4wgu-1741350288034"></a>[流处理](#nbcs-1721932419014)

<a name="m9ft-1741350288036"></a>[流处理的应用](#jf4x-1721932507881)

<a name="g3rt-1741350288038"></a>[复合时间处理](#80rp-1721932792357)

<a name="ojjx-1741350288040"></a>[流分析](#6jjt-1721932799443)

<a name="gdpm-1741350288042"></a>[维护物化视图](#5iog-1721932801692)

<a name="s2ht-1741350288044"></a>[在流上搜索](#0asi-1721932807278)

<a name="h667-1741350288046"></a>[消息传递和RPC](#10bm-1721932817553)

<a name="lfor-1741350288048"></a>[时间推理](#oca5-1721932519502)

<a name="lrq3-1741350288050"></a>[事件时间和处理时间](#is6l-1721932844579)

<a name="zsyw-1741350288052"></a>[知道什么时候准备好](#aue5-1721932851975)

<a name="pe8n-1741350288054"></a>[你用的是谁的时钟](#vsi1-1721932859473)

<a name="4yjf-1741350288056"></a>[窗口的类型](#gryf-1721932868260)

<a name="jenu-1741350288058"></a>[流式连接](#jbet-1721932523864)

<a name="kgkn-1741350288060"></a>[流流连接(窗口连接)](#eaux-1721932880085)

<a name="kkbm-1741350288062"></a>[流表连接(流扩展)](#hixz-1721932915139)

<a name="un3i-1741350288064"></a>[表表连接(维护物化视图)](#zbsv-1721932945841)

<a name="uf4c-1741350288066"></a>[连接的时间依赖性](#xqtb-1721932962310)

<a name="kkgr-1741350288068"></a>[容错](#v1om-1721932530009)

<a name="hn6k-1741350288070"></a>[微批量与存档点](#pqfe-1721932996846)

<a name="pgcv-1741350288072"></a>[原子提交再现](#elio-1721933013244)

<a name="0iud-1741350288074"></a>[幂等性](#bwyd-1721933023622)

<a name="qr5f-1741350288076"></a>[失败后重建状态](#dijm-1721933026909)

<a name="mdi0-1741350288078"></a>[12. 数据系统的未来](#vrhs-1739552413424)

<a name="hrri-1741350288080"></a>[数据集成](#ede5-1739555478378)

<a name="lf3s-1741350288082"></a>[组合使用衍生数据的工具](#72md-1739555630486)

<a name="yogn-1741350288084"></a>[理解流数据](#ws0u-1739555668111)

<a name="8he1-1741350288086"></a>[衍生数据与分布式事务](#zqyv-1739555677949)

<a name="c9z4-1741350288088"></a>[全局有序的限制](#sdr6-1739555687850)

<a name="hyvi-1741350288090"></a>[排序事件以捕捉因果关系](#zk4p-1739555695102)

<a name="oryn-1741350288092"></a>[批处理与流处理](#niva-1739555642700)

<a name="vszm-1741350288094"></a>[维护衍生状态](#r4mh-1739555731398)

<a name="oom2-1741350288096"></a>[应用演化后重新处理数据](#dx0m-1739555748166)

<a name="36oh-1741350288098"></a>[Lambda架构](#2vm7-1739555772929)

<a name="ki7f-1741350288100"></a>[统一批处理和流处理](#6flf-1739555780198)

<a name="3wl6-1741350288102"></a>[分拆数据库](#10z2-1739555560574)

<a name="clml-1741350288104"></a>[组合使用数据存储技术](#8wfw-1739555819342)

<a name="ethn-1741350288106"></a>[创建索引](#havr-1739555874428)

<a name="zxwt-1741350288108"></a>[一切的元数据库](#ep2e-1739555885454)

<a name="a9n4-1741350288110"></a>[开展分拆工作](#am3j-1739555961678)

<a name="uoeo-1741350288112"></a>[分拆系统和集系统](#trtv-1739555969624)

<a name="wvjp-1741350288114"></a>[少了什么？](#lcy6-1739555980725)

<a name="4lhr-1741350288116"></a>[围绕数据流设计应用](#97j6-1739555836550)

<a name="yt8m-1741350288118"></a>[应用代码所谓衍生函数](#gyfk-1739555997047)

<a name="4gzp-1741350288120"></a>[应用代码的状态和分离](#2njs-1739556023517)

<a name="8s3q-1741350288122"></a>[数据流：应用代码与状态变化的交互](#pgmn-1739556032065)

<a name="qa0k-1741350288124"></a>[流处理器和服务](#fyub-1739556046567)

<a name="x2hu-1741350288126"></a>[观察衍生数据状态](#dmmo-1739555844651)

<a name="8sfx-1741350288128"></a>[物化视图和缓存](#48oq-1739556059722)

<a name="ftoh-1741350288130"></a>[有状态，可离线的客户端](#srii-1739556068892)

<a name="bcfm-1741350288132"></a>[将状态变更推送给客户端](#zdkz-1739556083230)

<a name="je9b-1741350288134"></a>[端到端的事件流](#70bj-1739556096239)

<a name="wcwa-1741350288136"></a>[读也是事件](#k9nk-1739556109784)

<a name="w1us-1741350288138"></a>[多分区处理数据](#rknv-1739556117246)

<a name="3qr3-1741350288140"></a>[将事情做正确](#ydeg-1739555534326)

<a name="hsuf-1741350288142"></a>[为数据库使用端到端的参数](#bool-1739556139461)

<a name="ji4c-1741350288144"></a>[正好执行一次操作](#utq9-1739556271989)

<a name="zgf1-1741350288146"></a>[抑制重复](#mxfy-1739556285089)

<a name="yf2s-1741350288148"></a>[操作标识符](#9kug-1739556293281)

<a name="iykc-1741350288150"></a>[在数据系统中应用端到端的思考](#dc4e-1739556300938)

<a name="uxom-1741350288152"></a>[强制约束](#cai0-1739556155120)

<a name="5hzx-1741350288154"></a>[唯一性的约束要达成共识](#jeqp-1739556425758)

<a name="m7n6-1741350288156"></a>[基于日志消息传递中的唯一性](#lcq0-1739556436827)

<a name="gkf0-1741350288158"></a>[多分区请求处理](#dk4a-1739556687945)

<a name="mkmi-1741350288160"></a>[及时性与完整性](#dbka-1739556159893)

<a name="dcls-1741350288162"></a>[数据流系统的正确性](#bmr3-1739556705098)

<a name="mouz-1741350288164"></a>[宽松地解释约束](#loih-1739556715504)

<a name="roay-1741350288166"></a>[无协调数据系统](#v94l-1739556733932)

<a name="nrjl-1741350288168"></a>[信任但验证](#boru-1739556172269)

<a name="pmdu-1741350288170"></a>[维护完整性，尽管软件有bug](#zgrh-1739556749129)

<a name="xzlo-1741350288172"></a>[不要盲目信任任何承诺](#irhz-1739556777111)

<a name="cdur-1741350288174"></a>[验证的文化](#1d8z-1739556789067)

<a name="a227-1741350288176"></a>[为可审计性而设计](#61de-1739556792067)

<a name="xhee-1741350288178"></a>[端到端原则重现](#guck-1739556800394)

<a name="idqz-1741350288180"></a>[利用可审计数据系统的工具](#tjmv-1739556812856)

<a name="vzr7-1741350288182"></a>[做正确的事情](#dizf-1739555546308)

<a name="sukt-1741350288184"></a>[预测性分析](#va2h-1739556847417)

<a name="nsxj-1741350288186"></a>[偏见与分析](#c1wq-1739556910451)

<a name="aysd-1741350288188"></a>[责任与问责](#ijng-1739556917694)

<a name="y3b7-1741350288190"></a>[反馈循环](#thso-1739556928624)

<a name="l2mm-1741350288192"></a>[隐私和追踪](#gt9r-1739556854431)

<a name="f0mz-1741350288194"></a>[监视](#qol8-1739556939696)

<a name="8c9h-1741350288196"></a>[同意与选择的自由](#zgzn-1739556948482)

<a name="kdwp-1741350288198"></a>[隐私和数据使用](#ewvo-1739557089382)

<a name="prkb-1741350288200"></a>[数据资产与权力](#vlsj-1739557096182)

<a name="zw63-1741350288202"></a>[记着工业革命](#muzc-1739557108489)

<a name="sdbv-1741350288204"></a>[立法与法律](#7mjx-1739557115858)

<a name="pej1-1741350288206"></a>[本章小结](#hmh3-1739556886643)
...
<a name="ygxv-1740164523160"></a>**1.平衡树：写入时，添加到内存中平衡树结构（例如红黑树-二叉树），也称内存表**
...
<a name="dvbd-1740165554199"></a>**查找不存在的键时：LSM树算法很慢；检查内存表，一直回到最老的(可能必须从磁盘读取每一个)，才能确定键不存在**
...
<a name="wbca-1740441290207"></a>先内存再磁盘

<a name="nehk-1740441294133"></a>内存原地更新

<a name="amjs-1740441294135"></a>磁盘追加更新

<a name="8mpz-1740441294137"></a>归并保留新值
...
<a name="l62m-1740251469313"></a>**但是，较低的写入放大率和减少的碎片对SSD仍有利：更紧凑地表示数据可在可用的I/O带宽内提供更多读取和写入**
...
<a name="wg6v-1646311185918"></a>**1.单主复制（主流）**

<a name="ftqb-1646311200843"></a>**2.多主复制（弱一致性，难推理）**

<a name="ak8v-1646311613462"></a>客户端发送每个写入到几个领导节点之一，其中任何一个都可以接受写入。领导者将数据更改事件流发送给彼此以及任何跟随着节点。

<a name="4xbf-1646311410600"></a>**3.无主复制（弱一致性，难推理）**
...
<a name="nuso-1740494326580"></a>**同步synchronously    异步asychronously**

<a name="npwl-1740494327002"></a>**所有从库设置为同步复制：慢，低可用**

<a name="jazy-1740494327003"></a>**半同步semi synchronous 即至少在两个节点上拥有最新的数据副本：主库和同步从库    持久durable**
...
<a name="5ufz-1708443850274"></a>**从库失效：LSN追赶恢复cauht up**
...
<a name="z7mn-1708523814309"></a>**failover的麻烦：1.异步复制数据丢失(丢弃新旧主库未同步的写入冲突)**
...
<a name="k8yo-1708524004340"></a>**基于语句的复制(每个写入请求语句statement)** 

<a name="hcqe-1708524155632"></a>**记录每个写入请求语句staetment DDL,DML，语句日志传输给从库**
...
<a name="6uy2-1708524426849"></a> **mysql 如语句存在任何不确定性，切换到基于行的数据mix格式**

<a name="pdko-1740495061700"></a>**PS: mysql binlog statment或者mix格式**

<a name="ityn-1708524645902"></a><a name="rnmz-1708524013585"></a>**传输预写式日志(WAL)-崩溃恢复，数据一致性**

<a name="hze4-1708524498278"></a>**日志存储引擎SSTables LSM树  WAL日志** 

<a name="qzop-1708524541132"></a>**磁盘块的B树  先写 WAL/redolog,**

<a name="fh0m-1740495146523"></a>**使用WAL构建从库副本，零停机升级**

<a name="fawu-1740495098375"></a>**PostgreSQL和Oracle等使用**

<a name="hgyb-1740495199982"></a>**问题：WAL经常出现复制协议不允许版本不匹配，升级需要停机**

<a name="rh9a-1708524641979"></a><a name="a3ng-1708524040960"></a>**逻辑日志复制(基于行)**

<a name="jiiu-1708524648209"></a>**binlog 基于行格式保存**

<a name="fl7g-1740495875972"></a>**逻辑日志-保存最新记录**

<a name="5j1k-1740495804710"></a>**优点：易向后兼容，运行不同版本数据库软件甚至不同存储引擎**

<a name="k8me-1740495947858"></a> **易解析，易发送外部，如数仓-  即捕获数据变更 change data capture**

<a name="ezoi-1708524083973"></a><a name="64q9-1740496220916"></a>**基于触发器的复制(基于代码-复制冲突的解决移到应用程序)**

<a name="pvsc-1740496222567"></a> **优点：灵活性，处理冲突。冲突解决逻辑的复制移动到应用程序**

<a name="bmrk-1740496244819"></a>**缺点：开销高** 

<a name="5ujm-1740496423468"></a>   **比基于数据库内置复制更易出错**

<a name="nyjo-1708527704881"></a><a name="jamd-1708524904250"></a>**.复制延迟问题(解决方法都是反分布式，或者趋于单机一致性。数据一致性模型：**

<a name="tqyk-1740497732644"></a><https://jepsen.io/consistency>)

<a name="gcpt-1740497746531"></a> 严格序列化u

<a name="dkjr-1740497761572"></a>序列化u线性化u

<a name="pdgd-1740497793984"></a>可重复读u  快照读u顺序的p

<a name="ighl-1740509512361"></a>游标稳定u 单调原子视图t因果s

<a name="j0ic-1740509535747"></a>读已提交tPRAMs

<a name="hpag-1740509547995"></a>读未提交t按读写t单调读t  单调写t  读己所写s

<a name="hkp3-1740509815261"></a><a name="ozew-1740509943707"></a>故障时，可用性分析，当前节点根据一致性是否可用

<a name="9jpk-1740509863408"></a>不可用Unavailable:在某些类型的网络故障期间不可用。为了确保安全，一些或所有节点必须暂停操作，等待强一致性。

<a name="qj60-1740509859659"></a>粘性可用Sticky Available：在每个没有故障的节点上都可用，只要客户端只与相同的服务器通信，而不是切换到新的服务器。

<a name="amuq-1740509969902"></a>可用总数Total Available：即使网络完全关闭，每个无故障节点都可用。
...
<a name="ocet-1740497378721"></a>**PS:mycat  解决复制延迟策略**

<a name="meth-1740499554784"></a>**.智能路由：根据业务需求智能选择节点，如根据用户ID选择特定的数据库实例 / 所有强一致性操作都在主节点上**

目前已有的4中负载均衡策略

`  `mycat balance="1" -负载均衡策略，目前取值有4种，mysql没有官方的分布式分片方案。
...
<a name="bubg-1740497368102"></a>3.所有的读请求随机分发到writeHost对应的readHost上执行, writeHost不负担读压力
